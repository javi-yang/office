            ...
            if cell.value and "margin" in str(cell.value):
                ...
                while True:
                    ...
                    if margin_val is None:
                        break  # 如果为空，停止本次margin向下查找，继续下一个margin
                    try:
                    ...
                    if margin_num < 0 and freq_col:
                        ...
                        freq_cell.fill = green_fill  # 标记为绿色
                        margin_cell = ws.cell(row=scan_row, column=margin_col)
                        margin_cell.fill = red_fill  # 标记为红色
                        results.append([margin_num, freq_val])
                    scan_row += 1

    # 写入output.xlsx
    out_wb = openpyxl.Workbook()
    out_ws = out_wb.active
    out_ws.append(["Margin", "Frequency"])
    for row in results:
        out_ws.append(row)
    out_wb.save(output_path)
    wb.save(input_path)  # 保存原始文件以保留颜色标记
